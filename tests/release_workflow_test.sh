...
grep -Fq 'apk add --no-cache build-base cmake ninja file git tar' "$workflow"
grep -Fq 'manifest=$(tar -tzf "$archive")' "$workflow"
grep -Fq 'manifest=$(tar -tzf "$RUNNER_TEMP/assets/$prefix.tar.gz")' "$workflow"
grep -Fq 'test "$(printf' "$workflow"
grep -Fq '| wc -l)" -eq 5' "$workflow"
