...
Item {
    ...
    function ratio(value): real {
        return root.availableNumber(value) ? Number(value) : -1;
    }
    function percent(value): string {
        return root.availableNumber(value) ? qsTr("%1%").arg(Math.round(Number(value) * 100)) : "—";
    }
    ...
    function rate(value): string {
        if (!root.availableNumber(value))
            return "—";
        const bytesValue = Number(value);
        return bytesValue >= 1048576 ? qsTr("%1 MiB/s").arg((bytesValue / 1048576).toFixed(1)) : qsTr("%1 KiB/s").arg((bytesValue / 1024).toFixed(1));
    }
    function temperature(value): string {
        return root.availableNumber(value) ? qsTr("%1°C").arg(Math.round(Number(value))) : "—";
    }
    function uptime(value): string {
        ...
        return days > 0 ? qsTr("%1d %2h").arg(days).arg(hours % 24) : hours > 0 ? qsTr("%1h %2m").arg(hours).arg(minutes % 60) : qsTr("%1m").arg(minutes);
    }
    ...
    ColumnLayout {
            ...
            Repeater {
                ...
                delegate: Button {
                    ...
                    background: Dashboard.Frame {
                        ...
                        color: tab.activeFocus ? Theme.focusAccent : root.selectedIndex === tab.index ? Theme.primaryAccent : Theme.passiveBorder
                        corners: ({
        ...
        RowLayout {
            ...
            SystemMetricPanel {
                ...
                Layout.fillWidth: true
                Layout.fillHeight: true
                heading: root.selectedDevice && root.selectedDevice.networkInterfaceName.length > 0 ? qsTr("NET %1").arg(root.selectedDevice.networkInterfaceName) : qsTr("NETWORK")
