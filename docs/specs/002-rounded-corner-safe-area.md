...
The validated Waveshare product 25623 panel has rounded physical viewport corners that obscure framebuffer pixels. The page background should remain full bleed, while meaningful content and bordered surfaces need one shared, measured inset from the physical outer edges.
...
8. Set the final safe inset to `ceil(maximum per-corner safe inset) + 1 px`.
...
- The visible sidebar panel remains exactly 64 logical pixels wide and is inset from the physical left, top, and bottom edges by `Theme.displaySafeInset`.
- The page background remains full bleed beneath the rounded glass; the inset sidebar draws its complete bordered surface inside the safe area.
...
- Page content begins after the sidebar's left inset and complete 64 px width; the compact navigation rail centers its button frames horizontally and starts them 8 px below the panel's top edge.
...
- [x] The recorded final value equals `ceil(maximum per-corner safe inset) + 1 px`.
- [x] The calibration outline at the measured 9 px inset is completely visible at all four corners; the final 10 px token adds clearance.
...
- [ ] The inset sidebar panel is exactly 64 px wide and the page background remains edge-to-edge.
...
- Guard: 1 px
- Final `Theme.displaySafeInset`: 10 px
