...
- Borders, dividers, rails, badges, and detail rails remain navy-blue variants attached to their existing semantic theme roles. The offline badge uses the passive-border role. Faint chart guides preserve their existing 10% and 20% alpha hierarchy.
- Source SVGs use matching fallback colors for navigation and detail content. QML tinting remains authoritative when an icon is displayed through a control.
...
- QML tests lock the core palette and confirm the sidebar, device card, graph, and status badge continue to consume semantic theme tokens.
...
- Selected, focused, pressed, online, offline, chart, and empty-page states remain legible.
