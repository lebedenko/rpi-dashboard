...
- The endpoint join uses the same two-pixel feathered geometry as the curve core and extends from local X=−1, beneath the clipped curve, through the ring center at local X=6. It is drawn below a densely tessellated feathered annulus whose transparent outer band and plot-background inner transition produce a smooth circular outline. The overlaps leave no detached pixels, rasterization gaps, or visibly heavier connector segment.
...
- Endpoint scene-graph coverage verifies curve/join stroke parity, overlap across the clip boundary and ring center, outer and inner ring antialiasing bands, and unchanged marker transforms, visibility, and node reuse.
